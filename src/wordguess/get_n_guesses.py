...
def check_dict_validity(result_hist, corpus):
    ...
    for guess, result in result_hist.items():
        if not isinstance(guess, str) or not isinstance(result, str):
            raise TypeError(
                f"Passed key-value pair {guess}-{result} is not of type `string`"
            )
        if len(guess) != len(result):
            raise ValueError(
                f"result '{result}' length does not match input length for '{guess}'"
            )
        if len(guess) == 0:
        ...
        for pos in range(len(guess)):
            ...
            if res == "2":
                ...
                if final_twos[pos] and final_twos[pos] != letter:
                    raise ValueError(
                        f"Positional Conflict between {letter} and {final_twos[pos]} at position {pos}"
                    )
                else:
        ...
        for guess, result in result_hist.items():
            ...
            if valid:
                for letter in set(guess):
                    ...
                    if (
                        count < min_letter_counts[letter]
                        or count > max_letter_counts[letter]
                    ):
                        valid = False
                        break
